...
const MetricasManualesAuditor: React.FC = () => {
    ...
    const [auxPlantCapacity, setAuxPlantCapacity] = useState(
        constants?.detailedMetrics?.capacityMW ? constants.detailedMetrics.capacityMW * 1000 : 0
    );
    const [metricas, setMetricas] = useState(null);
    const [decimals, setDecimals] = useState(2);
    
    // Calcular métricas al cargar el componente y cuando cambian los constants
    useEffect(() => {
        if (constants?.detailedMetrics?.capacityMW) {
            const capacidadKW = constants.detailedMetrics.capacityMW * 1000;
            setAuxPlantCapacity(capacidadKW);
            const resultados = calcularMetricasManuales(capacidadKW);
            setMetricas(resultados);
        }
    }, [constants]);

    const handleCalculate = () => {
        const resultados = calcularMetricasManuales(auxPlantCapacity);
        setMetricas(resultados);
    };
    ...
    // Verificar si constants está cargado
                ...
                <div className="bg-gray-800 rounded p-2">
                    <div className="flex gap-2">
                        <Input
                            ...
                            type="number"
                            value={auxPlantCapacity}
                            onChange={(e) => setAuxPlantCapacity(Number(e.target.value))}
                            placeholder="Capacidad de planta (kW)"
                        />
                        <Button
